...
const Sidebar = ({ isOpen = false, onClose }: Props) => {
  return (
   ...
   {isOpen && (
  <div
    className="fixed inset-0 bg-black/20 backdrop-blur-md z-40 md:hidden transition-opacity duration-300"
    onClick={onClose}
  />
)}
...
<aside
  className={`
    bg-[#15234E] text-white w-60 md:w-64 p-6 flex flex-col
    fixed top-0 bottom-0 left-0 z-50
    transform transition-transform duration-300 ease-in-out
    ${isOpen ? 'translate-x-0 opacity-100' : '-translate-x-full opacity-0'}
    md:translate-x-0 md:opacity-100 md:relative md:z-auto
  `}
>
